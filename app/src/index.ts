...
const app = new Elysia()
  ...
  .derive(async ({ headers, jwt }) => {
    ...
    if (auth?.startsWith('Bearer ')) {
      try {
        ...
        if (p?.userId) return { user: { userId:p.userId as number, username:p.username as string, role: p.role as string } };
      } catch {}
    }
    return { user: null as null | { userId:number; username:string; role:string } };
  })
  ...
  .post('/sign_up', async ({ body }) => {
    ...
    if (!username || !password || !confirm_password) {
      return { success: false, message: '必填项不能为空' };
    }
    ...
    try {
      ...
      // 返回给前端的数据（和前端约定的 success/message 统一）
...
.post('/sign_in', async ({ body, jwt, set }) => {
  ...
  const verified = await Bun.password.verify(password, user.password_hash);
  if (!verified) {
    set.status = 401;
    return { success: false, message: '用户名/邮箱或密码错误' };
  }
...
// 登出接口
  .post('/sign_out', async ({ set }) => {
  // 对无状态 JWT，服务端不需要做额外操作
  // 只返回成功即可，前端删除 token 即可
  ...
  .post('/images', async ({ body, user, set }) => {
    ...
    if (file.size > 10*1024*1024) { set.status=413; return { message:'单张图片 ≤ 10MB' }; }
